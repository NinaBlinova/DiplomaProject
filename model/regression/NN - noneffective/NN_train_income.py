...
y = np.log1p(df['IncomeAmount'].clip(5_000, 500_000))
...
# 10. Сохранение
model.save("nn_income_model.keras")
joblib.dump(scaler, "scaler_income.joblib")
joblib.dump(encoder, "encoder_income.joblib")

print("Модель поступлений сохранена")
...
pd.DataFrame({
    'y_true': y_true,
    'y_pred': y_pred
}).to_csv("nn_income_predictions.csv", index=False)
